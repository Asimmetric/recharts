...
RELEASE_ID=$(curl --data "{\"tag_name\": \"v1.0.$COMMIT_COUNT\", \"target_commitish\": \"master\", \"name\": \"v1.0.$COMMIT_COUNT\", \"body\": \"Release for asimmetric recharts\", \"draft\": false, \"prerelease\": false}" https://api.github.com/repos/Asimmetric/recharts/releases?access_token=$GITHUB_AUTH_TOKEN | python -c "import sys, json; print json.load(sys.stdin)['id']")
